...
def async_database():
    with patch.object(
        settings, "database_uri", return_value="sqlite:///test.db"
    ), patch("AsyncDatabase.create_async_engine", new_callable=AsyncMock), patch(
        "AsyncDatabase.sessionmaker", new_callable=AsyncMock
